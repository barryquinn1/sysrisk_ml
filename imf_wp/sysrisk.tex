...
actions in Europe}
\author{Barry Quinn \and Barbara Casu \and Rym Ayadi \and Sami Ben Naceur \and Ronan Gallagher}
\date{May 10, 2021}
...
regression produces unacceptably noisy and biased estimates.  Furthermore, classical econometric method treat group-level effects as a nuisance parameter, removed via integration or marginalised to the error term. Our Bayesian multilevel
...
policy actions one to four quarters on. Interestingly, our models allow us to
directly compare effect sizes. Results show that loosening policy actions have the strongest
relative impact compared to tightening and ambiguous actions\footnote{Typically they have a 13 basis point reduction
...
banks, investment banks and brokerage, insurance and real estate covering 21
...
requirements (hereafter MCR) policy actions in this database\footnote{The decision to focus on only MCR actions is deliberate and theoretically motivated by collider bias issues first proposed by Pearl(2002). Conventional wisdom in classical econometric methods wrongly prioritise inclusion of statistical irrelevant as less damaging to statistical inference than exclusion of irrelevant variables. In modern econometric texts this problem is sometime referred to as \emph{Bad Controls}; in our context control for other prudential policy actions that can equally be consider as outcome variables in the analysis  \cite{angrist2008mostly,angrist2010credibility}}. To
...
\toprule
Activation of a new tool & Change in the level of an existing tool & Change in the scope of an existing tool & Deactivation of an existing tool & Maintaining the existing level and scope of a tool\\
\midrule
4 & 2 & 46 &  & 4\\
1 & 8 & 1 & 2 & \\
68 & 29 & 56 &  & 2\\
...
\begin{figure}[H]
\includegraphics{figures/paper-fig2-1} \caption{Posterior probability credible set MaP  CoVaR estimates}\label{fig:fig2}
\end{figure}
